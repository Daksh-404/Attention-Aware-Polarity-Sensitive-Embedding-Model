...
class NpairLoss(nn.Module):
    ...
    def forward(self, embeddings, target, n2, n4):
        ...
        anchor1 = torch.index_select(embeddings, 0, indices11)
        positive1 = torch.index_select(embeddings, 0, indices12)
        anchor2 = torch.index_select(embeddings, 0, indices21)                      ####CHANGED######
        positive2 = torch.index_select(embeddings, 0, indices22)
        ...
        anchor1_score_p = torch.index_select(polarity_score, 0, indices11)
        positive1_score_p = torch.index_select(polarity_score, 0, indices12)                ###CHANGED###
        anchor2_score_p = torch.index_select(polarity_score, 0, indices21)
        positive2_score_p = torch.index_select(polarity_score, 0, indices22)

        anchor1_score_e = torch.index_select(emotion_score, 0, indices11)
        positive1_score_e = torch.index_select(emotion_score, 0, indices12)                 ###CHANGED####
        anchor2_score_e = torch.index_select(emotion_score, 0, indices21)
        positive2_score_e = torch.index_select(emotion_score, 0, indices22)
        ...
        logit1 = logit1 * weight_polarity1
        logit2 = logit2 * weight_polarity2     ########## CHANGED###

        logit1 = logit1 * weight_emotion1
        logit2 = logit2 * weight_emotion2


        logit1_2 = torch.zeros(8, 2)            ###CHANGED###
        logit2_2 = torch.zeros(8, 2)
        ...
        tg2 = torch.zeros(8, 2)     #CHANGED###
        tg2[:, 0] = 1
        tg3 = torch.cat([torch.eye(4), torch.eye(4)], 0)      ###CHANGED##
